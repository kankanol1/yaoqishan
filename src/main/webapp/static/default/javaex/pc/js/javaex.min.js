  ...
  var javaex = function () {
    ...
    var info = {
      ...
      score: function (args) {
        ...
        var liHtml = '';
        for (var i = 1; i <= num; i++) {
          liHtml += '<li index="' + i + '" score="' + scoreArr[i - 1] + '"><i class="star icon-star-o" style="font-size:' + size + 'px;"></i></li>'
        }
        ...
        scoreChange = function (elem, index) {
          return $(elem).each(function (i, scoreElem) {
            return $(scoreElem).find("i").each(function (i, item) {
              if (i <= index) {
                $(item).removeClass("icon-star-o");
                return $(item).addClass("icon-star")
              } else {
                $(item).removeClass("icon-star");
                return $(item).addClass("icon-star-o")
              }
            })
          })
        };
      ...
      optTip: function (args) {
        ...
        if (type == "submit") {
          ...
          if ($(".tip-icon-loading").length > 0) {
            $(".opt-tip").css("background-color", "#2196f3");
            $(".opt-tip .tip-icon").removeClass("tip-icon-loading");
            $(".opt-tip .tip-icon").addClass("icon-check");
            $(".opt-tip .tip-icon").css({"font-size": "16px", "font-weight": "bold"});
            $(".opt-tip .tip-msg").text(opt.content)
            ...
            $(".opt-tip").remove();
            tipHtml += '<div class="opt-tip animated fadeInDown" style="top: 40px; left: 50%; background-color:#2196f3;">';
            tipHtml += '<div style="display: flex;">';
            tipHtml += '<span class="tip-icon icon-check" style="font-size:16px; font-weight:bold;"></span>';
            tipHtml += '<span class="tip-msg">' + opt.content + '</span>';
            tipHtml += '</div>';
      ...
      select: function (args) {
        ...
        if (opt.isSearch == true || opt.isSearch == "true") {
          ...
          $("#" + inputId).change(function () {
            ...
            if (maxSize == "" || maxSize == 0 || maxSize == "0") {
              ...
              if (imageFile.size > (maxSize * 1024 * 1024)) {
                info.optTip({content: "文件过大，单个文件上限 " + maxSize + "M", type: "error"});
                return false
              }
            }
            var data = new FormData();
